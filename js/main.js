(function (w, d) {
    ...
    var Blog = {
        ...
        fixedHeader: function (top) {
            if (top > header.clientHeight) {
                ...
                if (!window.matchMedia('(max-width: 1240px)').matches && !menu.classList.contains("hide")) {
                    $('.header-title').classList.add('fix-header-padding');
                }
            } else {
                header.classList.remove('fixed');
                header.classList.remove('higher-z');
                if (!window.matchMedia('(max-width: 1240px)').matches) {
                    $('.header-title').classList.remove('fix-header-padding');
                }
            }
        },
        toc: (function () {
    ...
    menuOff.addEventListener(even, function () {
        ...
        if ($('.header-title').classList.contains('fix-header-padding')){
            $('.header-title').classList.remove('fix-header-padding');
        }
    }, false);
                        ...
                        r => {
                            r.data.forEach(pubmed_obj => {
                                if (pubmed_obj.citation_count > 0) {
                                    ...
                                    document.querySelector(`#pubmed-citation-${pubmed_obj.pmid}`).innerText = pubmed_obj.citation_count;
                                } else {
